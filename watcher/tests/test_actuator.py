...
def test_plc_actuation_success(mock_client_cls):
    """Tests successful Modbus TCP relay activation."""
...
def test_plc_connection_failure(mock_client_cls, mock_fallback):
    """Tests behavior and fallback invocation when primary PLC Modbus TCP connection fails."""
    mock_client = MagicMock()
    ...
    mock_client.connect.assert_called_once()
    mock_fallback.assert_called_once_with(plc_config)
